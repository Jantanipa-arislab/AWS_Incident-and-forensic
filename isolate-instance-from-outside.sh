...
aws ec2 create-security-group --group-name isolation-sg --description “Security group to isolate a EC2-VPC instance" --vpc-id vpc-666666
# where vpc-666666 is the VPC ID that the instance is member of

#Set a rule to allow access from your IP only not R-Bung , but first we have to know our public IP:
dig +short myip.opendns.com @resolver1.opendns.com
aws ec2 authorize-security-group-ingress --group-name isolation-sg --protocol tcp --port xx --cidr YOUR.IP.ADDRESS.HERE/32
aws ec2 authorize-security-group-ingress --group-id sg-BLOCK-ID --protocol tcp --port xx --cidr YOUR.IP.ADDRESS.HERE/32 
